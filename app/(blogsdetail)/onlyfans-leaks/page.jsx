...
async function getData(perPage, page) {
  try {
    ...
    const db = client.db("onlyfansleaks");

    // DB Query
    const items = await db
      .collection("onlyfansleakscol")
      .find({})
      .skip(perPage * (page - 1))
      .limit(perPage)
      .toArray();

    const itemCount = await db.collection("onlyfansleakscol").countDocuments({});
...
export default async function Page({ searchParams }) {
  ...
  return (
    ...
    <li key={item._id} className="bg-green-500 rounded-md p-4 text-black">
        <Link href={`/onlyfans-leak/${item.url}`}>  {/* Link to the slug */}
            {item.title}
