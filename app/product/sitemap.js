...
export async function generateSitemaps() {
  ...
  const numSitemaps = Math.ceil(products.length / 1000); // Google's limit is 50,000 URLs per sitemap
  const sitemaps = [];
...
export default async function sitemap({ id }) {
  ...
  const start = id * 1000;
  const end = Math.min(start + 1000, products.length); // Ensure end doesn't exceed product count
