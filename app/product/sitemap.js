...
export async function generateSitemaps() {
  ...
  const numSitemaps = Math.ceil(products.length / 5000); // Google's limit is 50,000 URLs per sitemap
  const sitemaps = [];
...
export default async function sitemap({ id }) {
  ...
  const start = id * 5000;
  const end = Math.min(start + 5000, products.length); // Ensure end doesn't exceed product count
