...
export async function generateSitemaps() {
  ...
  const numSitemaps = Math.ceil(products.length / 50000); // Google's limit is 50,000 URLs per sitemap
  const sitemaps = [];
...
export default async function sitemap({ id }) {
  ...
  const end = Math.min(start + 50000, products.length); // Ensure end doesn't exceed product count
